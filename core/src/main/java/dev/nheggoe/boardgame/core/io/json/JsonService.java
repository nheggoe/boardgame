...
/// Service class for reading from and writing to JSON files. This class utilizes [JsonReader]
/// for deserializing JSON content into objects and [JsonWriter] for serializing objects into
/// JSON files. It offers functionality to handle collections of specified types.
///
/// The behavior of the JSON operations is determined by the associated class type and an optional
...
public class JsonService<T> implements DAO<T> {
  ...
  /// to production.
  ///
  /// @throws UnsupportedOperationException if the [JsonType] does not contain the necessary
