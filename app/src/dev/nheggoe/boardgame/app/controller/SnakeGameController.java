    ...
    switch (coreEvent) {
      ...
      case CoreEvent.GameEnded(Player winner) ->
          AlertFactory.createAlert(
                  AlertType.INFO, "%s has won the game!".formatted(winner.getName()))
              .showAndWait();
    }
  }
  ...
  private void onSnakeAndLadderEvent(SnakeAndLadderEvent snakeEvent) {
    switch (snakeEvent) {
      case SnakeAndLadderEvent.SnakeEncountered(int position) ->
          AlertFactory.createAlert(
                  AlertType.INFO, "Snake encountered at position %d!".formatted(position))
              .showAndWait();

      case SnakeAndLadderEvent.LadderEncountered(int position) ->
          AlertFactory.createAlert(
                  AlertType.INFO, "Ladder encountered at position %d!".formatted(position))
              .showAndWait();
    }
  }
  ...
  private void onUIEvent(UserInterfaceEvent uiEvent) {
    switch (uiEvent) {
      ...
      case UserInterfaceEvent.Alert(String message, var callback) -> {
        var result = AlertFactory.createAlert(AlertType.CONFIRM, message).showAndWait();
        callback.accept(
            result
